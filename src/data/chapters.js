export const chapters = [
  {
    id: 1,
    title: "Типы данных, сравнения и сортировка",
    subchapters: [
      {
        id: "1.1",
        title: "Основные типы данных",
        content: `
**Вывод информации в терминал**
print() - встроенная функция для вывода информации на экран
Она печатает всё, что можно выразить через символы: 
буквы, цифры, специальные символы, результаты подсчёта математических выражений, тексты ошибок и исключений 
Он пишется в конце программы или для проверки:

**Пример:** 
\`\`\`print(5) # в терминал выведет 5
\`\`\`

Типы данных — это способ классификации данных в Python.

**Основные типы:**
- int — целые числа
- float — числа с плавающей точкой
- str — строки (текст)
- bool — логические значения (True/False)
- list — изменяемый список
- tuple — неизменяемый список
- dict — словарь (ключ: значение)
- set — множество
- frozenset — неизменяемое множество

**Примеры:**
\`\`\`x = 5           # int
y = 3.14        # float
name = "Alice"  # str
flag = True     # bool
numbers = [1,2,3] # list
coords = (10,20)  # tuple
person = {"name": "Bob", "age": 25} # dict
unique = {1,2,3} # set
\`\`\`
        `
      },
      {
        id: "1.2",
        title: "Сравнение и логика",
        content: `
В Python доступны следующие **операторы сравнения**:

- ==  (равно)
- !=  (не равно)
- <   (меньше)
- >   (больше)
- <=  (меньше или равно)
- >=  (больше или равно)

**Пример:**
\`\`\`a = 10
b = 5

print(a > b)   # True
print(a == b)  # False
\`\`\`

**Логические операторы:**

- and — и
- or  — или
- not — не

**Пример:**
\`\`\`x = True
y = False

print(x and y) # False
print(x or y)  # True
print(not x)   # False
\`\`\`
        `
      },
      {
        id: "1.3",
        title: "Сортировка и ключи сортировки",
        content: `
Python предоставляет встроенные методы для сортировки.

- sorted(iterable, key=..., reverse=...) — возвращает новый отсортированный список
- list.sort(key=..., reverse=...) — сортирует список на месте

**Примеры:**
\`\`\`numbers = [5, 2, 9, 1]

# отсортированный список
sorted_numbers = sorted(numbers)
print(sorted_numbers)  # [1, 2, 5, 9]

# сортировка списка на месте
numbers.sort(reverse=True)
print(numbers)         # [9, 5, 2, 1]

# сортировка сложных объектов по ключу
students = [{"name": "Alice", "age": 20}, {"name": "Bob", "age": 18}]
students_sorted = sorted(students, key=lambda s: s["age"])
print(students_sorted)
# [{'name': 'Bob', 'age': 18}, {'name': 'Alice', 'age': 20}]
\`\`\`
        `
      }
    ]
  },
  {
    id: 2,
    title: "Управляющие конструкции и логика программы",
    subchapters: [
      {
        id: "2.1",
        title: "Условный оператор if",
        content: `
Оператор if выполняет код, если условие имеет значение True.

**Пример:**
\`\`\`age = 18

if age >= 18:
    print("Доступ разрешён")
\`\`\`
Если условие ложно, код внутри блока if не выполняется.
        `
      },
      {
        id: "2.2",
        title: "else и elif",
        content: `
**else** выполняется, если условие if не выполнено.

\`\`\`age = 15

if age >= 18:
    print("Доступ разрешён")
else:
    print("Доступ запрещён")
\`\`\`

**elif** позволяет проверять несколько условий последовательно.

\`\`\`score = 75

if score >= 90:
    print("Отлично")
elif score >= 70:
    print("Хорошо")
else:
    print("Нужно подтянуть знания")
\`\`\`
        `
      },
      {
        id: "2.3",
        title: "Логические операторы",
        content: `
Логические операторы объединяют условия:
- and — оба условия истинны
- or — истинно хотя бы одно условие
- not — инвертирует значение

\`\`\`age = 20
has_passport = True

if age >= 18 and has_passport:
    print("Можно ехать")
\`\`\`
        `
      },
      {
        id: "2.4",
        title: "Цикл while",
        content: `
Цикл while выполняется, пока условие истинно.

\`\`\`count = 0

while count < 5:
    print(count)
    count += 1
\`\`\`
Важно следить, чтобы условие выхода было достижимо.
        `
      },
      {
        id: "2.5",
        title: "Цикл for",
        content: `
Цикл for используется для перебора последовательностей.

\`\`\`for i in range(5):
    print(i)
\`\`\`

Перебор списка:

\`\`\`fruits = ["яблоко", "банан", "груша"]

for fruit in fruits:
    print(fruit)
\`\`\`
        `
      },
      {
        id: "2.6",
        title: "break и continue",
        content: `
**break** полностью прекращает выполнение цикла.

\`\`\`for i in range(10):
    if i == 5:
        break
    print(i)
\`\`\`

**continue** пропускает текущую итерацию.

\`\`\`for i in range(5):
    if i == 2:
        continue
    print(i)
\`\`\`
        `
      },
      {
        id: "2.7",
        title: "Оператор pass",
        content: `
Используется, когда блок кода синтаксически необходим, но логика ещё не реализована.

\`\`\`if True:
    pass
\`\`\`
        `
      },
      {
        id: "2.8",
        title: "Вложенные условия и циклы",
        content: `
Условия и циклы могут быть вложены друг в друга.

\`\`\`for i in range(3):
    for j in range(2):
        print(i, j)
\`\`\`

Пример с условием внутри цикла:

\`\`\`for i in range(5):
    if i % 2 == 0:
        print(i, "— чётное")
\`\`\`
        `
      },
      {
        id: "2.9",
        title: "Практические примеры",
        content: `
Проверка пароля:
\`\`\`password = "1234"
user_input = input("Введите пароль: ")

if user_input == password:
    print("Вход выполнен")
else:
    print("Неверный пароль")
\`\`\`

Подсчёт суммы чисел:
\`\`\`total = 0

for i in range(1, 6):
    total += i

print(total)
\`\`\`
        `
      }
    ]
  },
  {
    id: 3,
    title: "Функции в Python: как создавать и использовать свои функции с помощью def",
    subchapters: [
      {
        id: "3.1",
        title: "Зачем нужны функции?",
        content: `

Функции нужны, чтобы не повторять один и тот же код много раз
Они помогают сделать программу короче, понятнее и удобнее для исправления ошибок
Если нужно изменить действие — достаточно изменить его в одном месте

Функция позволяет:
• Повтор использование кода
• Упрощение и структурирование программ
Пример без функции (повтор кода)

\`\`\`print("Привет, Никита")
print("Привет, Никита")
print("Привет, Никита")
\`\`\`

Пример с функцией:
\`\`\`def say_hello():
    print("Привет, Никита")

say_hello()
say_hello()
say_hello()
\`\`\`

`
      },
      {
        id: "3.2",
        title: "Что такое функция в Python?",
        content: 
        `

Функция — это именованный блок кода, который выполняет определённое действие.
В Python есть:

Встроенные функции (print, len, input и др.)
Пользовательские функции, которые мы создаём сами.

Встроенная функция:
\`\`\`print("Hello")
\`\`\`

Пользовательская функция:
\`\`\`def greet():
    print("Hello")
\`\`\`
`
      },
      {
        id: "3.3",
        title: "Синтаксис создания функций (def)",
        content: `
Простейшая фукнция может просто выполнять какое-то действие,
например выводить сообщение.Такая фукнция не принимает параметры 
и не влзвращает результат

Важно понимать разницу между:
• **созданием фукнций** - когда мы пишем def...
• **вызовом функции** - когда мы пишем её имя с круглыми скобками
Пример структуры функции:
\`\`\`def my_function():
    print("это тело функции")
\`\`\`

def - это объявление функции
my_function - имя функции (лучше называть с маленькой буквы)
() - место для параметров
: - начало тела функции
      `
      },
      {
        id: "3.4",
        title: "Параметры и аргументы",
        content: `


Очень часто фукнция должна работать с разными данными.
Для этого используются **параметры**

• **Параметры** - это переменные, которые записываются 
при создания фукнции
• **Аргументы** - это реальные значения, которые передаются
при вызове функций

Проще говоря:
• Параметры - это "места для данных"
• аргументы - это сами данные

Функция может принимать:
• один параметр
• несколько параметров
• или не принимать ни одного

Функция без параметров:
\`\`\`def hello():
    print("Привет")
\`\`\`
Здеь:
• name - параметр
• "Маша" - аргумент
один параметр:

\`\`\`def hello(name):
    print("привет", name)
hello("маша")
\`\`\`

несколко параметров:
\`\`\`def add(a, b):
    print(a + b)

add(3, 5)
\`\`\`
функция получает два числа и выводит их сумму 
`     
      },
      {
        id: "3.5",
        title: "Возврат значения из функции (retern)",
        content: `


Иногда фукнция должна не просто что-то напечатать, а **вернуть результат**,
чтобы его можно было использовать дальше в программе

Для этого используеся команда **retern**

Когда Python встречает retern:
• Фукнция сразу заканчивает работу
• Указанное значение передается туда, где фукнция была вызвана

Если в функции нет retern, она автоматически возвращает значение **None**

функция с retern:

\`\`\`def square(x):
    return x * x

result = square(4)
print(result)
\`\`\`

Возврат нескольких значений:

\`\`\`def numbers():
    return 1, 2

a, b = numbers()
print(a, b)
\`\`\`
`
      },
      {
        id: "3.6",
        title: "Локальные и глобальные перемнные",
        content: `


У каждой перемнной есть **область видимости** - место в программе,
где её можно использовать 

• **Локальные переменные** создаются внутри функции и существуют только там
• **Глобальные переменные** создаются вне фукнций и доуступны почти везде

Это сделано для того,чтобы разные части программы не мешали друг другу
Локальная переменная:
\`\`\`def test():
    x = 5
    print(x)

test()
# print(x)  # Ошибка: x не существует вне функции
\`\`\`

Глобальная переменная:
\`\`\`y = 10

def show():
    print(y)

show()
\`\`\`
 `       
      },
      {
        id: "3.7",
        title: "Документация функции (docstring)",
        content: `


**docstring** - это строка с описанием функции. Она нужна, чтобы:
• объяснить, что делает фукнция
• какие параметры она принимает
• что она возвращает

Пример с описанием функции:
\`\`\`def add(a, b):
    """Функция складывает два числа"""
    return a + b

print(add.__doc__)
\`\`\`        `       
      },
      {
        id: "3.8",
        title: "Частые ошибки при создании фукнций",
        content: `


**Новички часто делают похожие ошибки:**
1.Забыают retern, когда нужен результат
2.Делают неправильные отступы
3.Путают имена фукнций

Забыли retern:
\`\`\`  def add(a, b):
    a + b

print(add(2, 3))  # None
\`\`\`  

Ошибка в отступах:
\`\`\` def hello():
    print("Привет")  # Ошибка IndentationError (синтаксическая)
\`\`\` 

Ошибка имени:
\`\`\` def Hello():
    print("Привет")

hello()  # Ошибка: имя другое
\`\`\` 
        `        
      },
      {
        id: "3.9",
        title: "практические задания",
        content: ` 


**практика позволяет лучше понять:**
• как работает параметры
• как возвращаются значения
• как правильно вызывать фукнции

задание 1: функция,которая выводит имя
\`\`\` def show_name(name):
    print("Твоё имя:", name)

show_name("Ира")
\`\`\` 

задание 2: функция сложения
\`\`\` def sum_numbers(a, b):
    return a + b

print(sum_numbers(7, 8))
\`\`\` 

        `        
      }
    ]
  },
    {
    id: 4,
    title: "Списки в Python",
    subchapters: [
      {
        id: "4.1",
        title: "Что такое список?",
        content: `


**Список в Python** — это упорядоченная изменяемая коллекция объектов произвольных типов.

**Основные свойства списков:**

- **Упорядоченность**: элементы хранятся в том порядке, в котором они были добавлены.
- **Изменяемость**: можно добавлять, удалять и изменять элементы после создания списка.
- **Индексируемость**: к каждому элементу можно обратиться по его позиции (индексу).
- **Допускают дубликаты**: один и тот же элемент может встречаться в списке несколько раз.
        `
      },
      {
        id: "4.2",
        title: "Какие списки бывают? Способы создания",
        content: `
Существует несколько способов создания списков в Python:

**1) С помощью квадратных скобок \`[]\`:**
\`\`\`
# Пустой список
empty_list = []
print(empty_list)

# Список чисел
numbers = [1, 2, 3, 4, 5]
print(numbers)

# Список разных типов данных
mixed = [1, "привет", True, 3.14]
print(mixed)

# Вложенные списки (список списков)
matrix = [[1, 2, 3], [4, 5, 6], [7, 8, 9]]
print(matrix)
\`\`\`

**2) С помощью конструктора \`list()\`:**
\`\`\`
# Создание пустого списка
empty_list = list()
print(empty_list)

# Создание списка из строки (каждый символ становится элементом)
chars = list("Python")
print(chars)  # ['P', 'y', 't', 'h', 'o', 'n']

# Создание списка из других итерируемых объектов
tuple_to_list = list((1, 2, 3))
print(tuple_to_list)

set_to_list = list({1, 2, 3})
print(set_to_list)
\`\`\`
        `
      },
      {
        id: "4.3",
        title: "Доступ к элементам списка",
        content: `
**Индексация:**
В Python индексация начинается с 0.

\`\`\`p
fruits = ["яблоко", "банан", "вишня", "груша", "апельсин"]

# Получение элементов по индексу
first_fruit = fruits[0]
print(f"Первый фрукт: {first_fruit}")  # яблоко

# Отрицательные индексы для доступа с конца списка
last_fruit = fruits[-1]
print(f"Последний фрукт: {last_fruit}")  # апельсин

second_last = fruits[-2]
print(f"Предпоследний фрукт: {second_last}")  # груша
\`\`\`

**Срезы:**
Синтаксис: \`список[начало:конец:шаг]\`  
(Начало включается, конец — нет!)

\`\`\`
fruits = ["яблоко", "банан", "вишня", "груша", "апельсин"]

# Первые три элемента
first_three = fruits[:3]
print(first_three)  # ['яблоко', 'банан', 'вишня']

# Каждый второй элемент
every_second = fruits[::2]
print(every_second)  # ['яблоко', 'вишня', 'апельсин']

# Разворот списка
reversed_list = fruits[::-1]
print(reversed_list)  # ['апельсин', 'груша', 'вишня', 'банан', 'яблоко']
\`\`\`

**Изменение элементов:**
\`\`\`
fruits = ["яблоко", "банан", "вишня"]
fruits[0] = "киви"
print(fruits)  # ['киви', 'банан', 'вишня']

# Изменение через срез
numbers = [1, 2, 3, 4, 5]
numbers[1:4] = [20, 30]
print(numbers)  # [1, 20, 30, 5]
\`\`\`
        `
      },
      {
        id: "4.4",
        title: "Основные методы списков",
        content: `


**1) Добавление элементов:**
\`\`\`\
fruits = ["яблоко", "банан"]

fruits.append("вишня")           # в конец
fruits.insert(1, "апельсин")     # на позицию 1
more_fruits = ["груша", "виноград"]
fruits.extend(more_fruits)       # добавить другой список
combined = fruits + ["ананас"]   # объединение списков
\`\`\`

**2) Удаление элементов:**
\`\`\`
fruits = ["яблоко", "банан", "вишня", "банан"]

fruits.remove("банан")           # удаляет первое вхождение
removed = fruits.pop(1)          # удаляет по индексу и возвращает
last = fruits.pop()              # удаляет последний
fruits.clear()                   # очистить весь список

# Также можно использовать del
numbers = [1, 2, 3, 4, 5]
del numbers[0]        # удаляет по индексу
del numbers[1:3]      # удаляет срез
\`\`\`

**3) Поиск и подсчёт:**
\`\`\`
fruits = ["яблоко", "банан", "вишня", "банан"]

print("банан" in fruits)         # True
index = fruits.index("банан")    # 1
count = fruits.count("банан")    # 2
\`\`\`

**4) Сортировка и разворот:**
\`\`\`
numbers = [3, 1, 4, 1, 5]

numbers.sort()                   # сортирует на месте: [1, 1, 3, 4, 5]
numbers.sort(reverse=True)       # по убыванию

# Без изменения исходного списка:
original = [3, 1, 4]
sorted_copy = sorted(original)   # [1, 3, 4]

# Разворот
numbers.reverse()                # или numbers[::-1]
\`\`\`
        `
      },
      {
        id: "4.5",
        title: "Практический пример использования списка",
        content: `
\`\`\`
# Список студентов и их оценок
students = ["Анна", "Иван", "Мария", "Петр", "Елена"]
grades = [95, 82, 90, 78, 88]

# Нахождение студента с наивысшим баллом
highest_score = max(grades)
top_student_index = grades.index(highest_score)
print(f"Лучший студент: {students[top_student_index]} с оценкой {highest_score}")
# Вывод: Лучший студент: Анна с оценкой 95
\`\`\`
        `
      }
    ]
  }
]